...
## 📶 4. Capa de Red Local: Cliente Ligero (BridgeClient)
Para que el Móvil no requiera dependencias masivas o bases de datos redundantes que puedan romperse con el OS base, esta app utiliza `BridgeClient` enlazado con Python nativo `urllib`.

- **Descubrimiento Transparente**: Se utiliza `zeroconf` en Python como componente buscador del dispositivo en red publicitado dinámicamente bajo `_keyvault._tcp.local.`, previniendo así un requerimiento de *IP estática*.
- **Tunneling de Confianza**: Los Handshakes inician por una semilla conformada via SHA-256 integrando un código numérico y una Key Alfa revelada solamente en el Server (*Out of Band Security*).
- **Control Unidireccional de Portapapeles**: Las rutas se exponen interconectando la función OS genérica nativa con las apis REST protegidas bajo el cifrado asimétrico persistente AES-256.
